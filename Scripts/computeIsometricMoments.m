function [moments, angles] = computeIsometricMoments(model,angles,fixed_angle,coord_name,fixed_coord_name,fle_muscles,ext_muscles)
        ...
        for m = 1:muscleNames.size
            if any(strcmp([fle_muscles ext_muscles],string(muscleNames.get(m-1))))
                ...
                if any(strcmp(fle_muscles,string(muscleNames.get(m-1))))
                    ...
                    active_moments(c) = active_moments(c) + moment_arm(muscle_all,c)*passive_force(muscle_all,c);    
                end
